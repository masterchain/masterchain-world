...
def send_form_response(response_dict):
    ...
    btc_fee=get_response_field(response_dict, 'fee')
    try:
    ...
    add_marker=False
    try:
        marker=get_response_field(response_dict, 'marker')
        info(marker)
